"""Douglas County Clerk & Recorder adapter for Landmark Web system"""
...
@register_adapter("douglas")
class DouglasCountyAdapter(BaseCountyAdapter):
    """
    Adapter for Douglas County Clerk & Recorder - Landmark Web system.

    Portal: https://apps.douglas.co.us/LandmarkWeb
    ...
    Notes:
        - Free 24/7 online search access
        - Account creation required for document copies
        - Fraud detection service available
    """

    BASE_URL = "https://apps.douglas.co.us/LandmarkWeb"
    ...
    async def initialize(self, page: Page) -> bool:
        ...
        try:
            self.logger.info("Initializing Douglas County Landmark Web adapter")
            ...
            if "Search" in content or "landmarkweb" in page.url.lower():
                ...
                self.logger.info("Douglas County adapter initialized successfully")
                return True
            else:
            ...
            self.logger.error(f"Failed to initialize Douglas adapter: {e}")
            await self.screenshot_on_error(page, "douglas_init_error")
            return False
    ...
    async def search_by_name(
        ...
        """Search Douglas County records by grantor/grantee name"""
        results = []

        try:
            self.logger.info(f"Searching Douglas County by name: {name}")
            ...
            self.logger.error(f"Douglas name search failed: {e}")
            await self.screenshot_on_error(page, "douglas_name_search_error")
    ...
    async def search_by_instrument(
        ...
        """Search Douglas County records by reception/instrument number"""
        results = []

        try:
            self.logger.info(f"Searching Douglas County by instrument: {instrument_number}")
            ...
            self.logger.error(f"Douglas instrument search failed: {e}")
            await self.screenshot_on_error(page, "douglas_instrument_search_error")
    ...
    async def search_by_parcel(
        ...
        """Search Douglas County records by parcel/legal description"""
        self.logger.info(f"Parcel search requested for: {parcel_number}")
        self.logger.warning("Douglas County Landmark Web requires legal description search. "
    ...
    async def download_document(
        ...
        """Download a document from Douglas County"""
        try:
            ...
            self.logger.warning("Note: Douglas County may require account creation for document copies")
    ...
    async def check_health(self) -> bool:
        """Check if the Douglas County portal is accessible."""
        try:
            import httpx
