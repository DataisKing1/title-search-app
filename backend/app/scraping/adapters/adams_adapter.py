"""Adams County Clerk & Recorder adapter for KoFile County Fusion system"""
...
@register_adapter("adams")
class AdamsCountyAdapter(BaseCountyAdapter):
    """
    Adapter for Adams County Clerk & Recorder - KoFile County Fusion system.

    Portal: https://countyfusion2.kofiletech.us/countyweb/loginDisplay.action?countyname=Adams
    ...
    Note: Very similar to Denver County which uses County Fusion 3.
    """

    BASE_URL = "https://countyfusion2.kofiletech.us/countyweb"
    LOGIN_URL = f"{BASE_URL}/loginDisplay.action?countyname=Adams"
    GUEST_LOGIN_URL = f"{BASE_URL}/login.action?countyname=Adams&guest=true"
    ...
    async def initialize(self, page: Page) -> bool:
        ...
        try:
            self.logger.info("Initializing Adams County KoFile adapter")
            ...
            self.logger.info("Adams County adapter initialized successfully")
            return True

        except Exception as e:
            self.logger.error(f"Failed to initialize Adams adapter: {e}")
            await self.screenshot_on_error(page, "adams_init_error")
            return False
    ...
    async def search_by_name(
        ...
        """Search Adams County records by grantor/grantee name"""
        results = []

        try:
            self.logger.info(f"Searching Adams County by name: {name}")
            ...
            self.logger.error(f"Adams name search failed: {e}")
            await self.screenshot_on_error(page, "adams_name_search_error")
    ...
    async def search_by_instrument(
        ...
        """Search Adams County records by reception/instrument number"""
        results = []

        try:
            self.logger.info(f"Searching Adams County by instrument: {instrument_number}")
            ...
            self.logger.error(f"Adams instrument search failed: {e}")
            await self.screenshot_on_error(page, "adams_instrument_search_error")
    ...
    async def search_by_parcel(
        ...
        """Search Adams County records by parcel number - not directly supported"""
        self.logger.info(f"Parcel search requested for: {parcel_number}")
        self.logger.warning("Adams County KoFile doesn't support direct parcel search. "
    ...
    async def download_document(
        ...
        """Download a document from Adams County"""
        try:
    ...
    async def check_health(self) -> bool:
        """Check if the Adams County portal is accessible."""
        try:
            import httpx
            async with httpx.AsyncClient() as client:
                response = await client.get(
